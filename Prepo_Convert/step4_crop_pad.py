...
path_in = Path("D:/Skripsi/newprepo/preprocessed/data") # Path ke folder hasil export sebelumnya
files = list(path_in.rglob("img.nii.gz")) # Baca semua file img.nii.gz

print("Jumlah img.nii.gz ditemukan:", len(files))
for f in files:
    print("➡️", f)
    
path_root = Path('D:/Skripsi/newprepo') # Path ke folder project
path_in = path_root/'preprocessed/data'    # Path ke folder hasil export sebelumnya
path_out = path_root/'preprocessed_crop/data' # Output path hasil crop
