...
# # Ambil satu scan untuk test
# scan = pl.query(pl.Scan).first()

# # Tampilkan path folder DICOM-nya
# print("Path ke DICOM:", scan.get_path_to_dicom_files())

# Setting path
path_root = Path('D:/Skripsi/newprepo') # Path ke folder project
path_root_in = 'D:/Skripsi/newprepo/LIDC-IDRI' # Path ke folder LIDC-IDRI
path_root_out = path_root/'preprocessed' # Output path file setelah preprocessing
...
if __name__ == "__main__":
    ...
    ## Get all scans 
    ## pakai kode ini kalau punya seluruh data LIDC
    # scan_ids = range(1, len(list(pl.query(pl.Scan)))+1) # scan semua data

    # Ambil scan data pasien 001 -011
    selected_ids = [
        scan.id
